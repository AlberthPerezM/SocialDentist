...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 24-04-2022 a las 07:10:05
...
(1, 'Extracciones', 'Es un procedimiento que consiste en extraer el diente enfermo que no puede ser tratado a través de otros métodos conservativos, para evitar posibles infecciones graves.La extracción del diente puede ser simple o quirúrgica, el procedimiento más común utilizado para la extracción de los dientes es la extracción simple. Si para extraer el diente es necesaria una fuerza excesiva para obtener la extracción del diente, o si falta una parte sustancial de la corona del diente y el acceso a la raíz se vuelve difícil, entonces se recomienda la extracción quirúrgica del diente.', 50, 'https://fundacionbest.org.mx/assets/images/images_notas/extraccion-dental.jpg'),
...
INSERT INTO `usuario` (`id_usuario`, `nombreUsuario`, `apellidoPusuario`, `apellidoMusuario`, `contraseña`, `foto`, `id_cargo`) VALUES
(1, 'Efrain', 'Tito', 'Camones', '154859', 'https://img.icons8.com/external-konkapp-outline-color-konkapp/200/external-dentist-profession-avatar-konkapp-outline-color-konkapp.png', 1),
(2, 'Karolina', 'Hernandez', 'Zapata', '668956', 'https://img.icons8.com/external-konkapp-outline-color-konkapp/200/external-nurse-emergency-services-konkapp-outline-color-konkapp.png', 2);
...
--
ALTER TABLE `consulta`
  ADD CONSTRAINT `consulta_ibfk_1` FOREIGN KEY (`id_especialidad`) REFERENCES `especialidad` (`id_especialidad`);
